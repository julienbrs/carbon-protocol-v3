...
URI="ipfs://QmSFXecKEpEZ8eKiMPjmgjVZgNAx6JE6NTg8AionJPZVV7/{id}.json"
...
deploy() {
    ...
    sleep 5
    if [[ $debug == "true" ]]; then
        printf "deploy %s %s %s %s %s %s \n" "$class_hash" "$URI" "$OWNER" "$STARTING_YEAR" "$NUMBER_OF_YEARS" >> debug_project.log
    fi
    output=$(starkli deploy $class_hash str:"$URI" "$OWNER" $STARTING_YEAR $NUMBER_OF_YEARS --keystore-password $KEYSTORE_PASSWORD --watch 2>&1)
...
setup() {
    ...
    sleep 15
    if [[ $debug == "true" ]]; then
        printf "Deployed contract: %s\n" "$contract" >> debug_project.log
    fi
    
    if [[ $debug == "true" ]]; then
        printf "call invoke %s set_project_carbon \n" "$contract" >> debug_project.log
    fi
...
printf "contract deployed at: %s\n" "$contract_address" >> debug_project.log